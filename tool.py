...
argParser = argparse.ArgumentParser(
    prog="DomainCheckr.py",
    description="Analyze IP Addresses and domains using Virus Total and Abuse IPDB APIs.",
    epilog=(
        "If you won't input anything, the code will run using the target-list.txt "
        "using VirusTotal and AbuseIPDB. You can input domains, but they will be "
        "only checked on Virus Total because AbuseAPI does not support domains."
    ),
)
argParser.add_argument(
    ...
    "--version",
    action="store_true",
    help="Prints current version of the script and exit.",
    required=False,
)
argParser.add_argument(
    "-i",
...
argParser.add_argument(
    ...
    "--virustotalreports",
    type=int,
    help="Virustotal malicious reports threshold. Default 1.",
    required=False,
    default=1,
...
argParser.add_argument(
    ...
    "--abusereports",
    type=int,
    help="Abuse IP DB malicious reports threshold. Default 1.",
    required=False,
    default=1,
    metavar="[Threshold number]",
)
argParser.add_argument(
    ...
    type=str,
    help=(
        "Provide the path of the txt file containing targets. "
...
argParser.add_argument(
    ...
    type=int,
    help=(
        "Use multiple threads. It is recommended to use it if you have licensed API keys. "
        "Also, CLI output is buggy when used. I recommend using it with -r."
    ),
    required=False,
    default=1,
    metavar="[Threads]",
...
argParser.add_argument(
    ...
    "--whois",
    action="store_true",
    help="Run whois analysis on the provided targets.",
    required=False,
)
...
if args.whois:
    ...
    def extract_domains_from_requirements(file_path):
        ...
        with open(file_path, "r") as file:
            for line in file:
                ...
                domains.append(
                    f"{package_name}"
                )  # Append ".com" for simplicity, you may need a more accurate method
        return domains
    ...
    def check_domain_info(api_key, domains):
        ...
        for domain in domains:
            api_url = (
                f"{api_endpoint}?apiKey={api_key}&domainName={domain}&outputFormat=JSON"
            )
            ...
            try:
                ...
                if response.status_code == 200 and data.get("WhoisRecord", {}).get(
                    "registryData"
                ):
                    ...
                    result = {
                        ...
                        "whois_data": {
                            ...
                            "Registration Date": registry_data.get(
                                "createdDate", "N/A"
                            ),
                ...
                results.append(
                    {"domain": domain, "whois_data": None, "error": f"Error: {str(e)}"}
                )
    ...
    whois_domains_to_check = extract_domains_from_requirements(
        whois_requirements_file_path
    )
...
def checkAbuse(value, counter):
    try:
        if ipPattern.search(value):
            ...
            querystring = {"ipAddress": value, "maxAgeInDays": "90"}
            abuse_headers = {
                "Accept": "application/json",
                "Key": abuseApiKeys[counter % len(abuseApiKeys)],
            }

            abuse_response = requests.request(
                method="GET",
                url=abuse_url,
            ...
            ispName, usageType = (
                "Can only check valid IP addresses on AbuseIPDB",
                "Can only check valid IP addresses on AbuseIPDB",
            )
...
def checkVirustotal(value, counter):
    try:
        ...
        isMalicious = json_file["data"]["attributes"]["last_analysis_stats"][
            "malicious"
        ]
...
def checking(value, counter):
    ...
    if (
        int(isMalicious) >= args.virustotalreports
        or int(reportCount) >= args.abusereports
        or reportScore == 100
    ) and usageType != "Reserved":
        ...
        vtHtml = """
            ...
            <tr>
                <td colspan="2" align="center">
                    <strong><a href="https://www.virustotal.com/gui/search/{address}" target="_blank">Virus Total</a></strong>
                </td>
            </tr>
            <tr>
                <td>AS Owner:</td>
            ...
            <tr>
                ...
                <td>
                    <table>
                        {''.join(
                            f'<tr><td>{engine}</td><td>{result["category"]}</td></tr>'
                            if isinstance(result, dict)
                            else f'<tr><td>{engine}</td><td>{result}</td></tr>'
                            for engine, result in lastAnalysisStats.items()
                        )}
                    </table>
                </td>
    ...
    if not args.abuse:
        abuseHtml = """
            <tr>
                <td colspan="2" align="center">
                    <strong><a href="https://www.abuseipdb.com/check/{address}" target="_blank">Abuse IP DB</a></strong>
                </td>
            </tr>
            <tr>
                <td><b>Address:</b></td>
    ...
    if abuseError or vtError:
        failedValues.append(address)
    else:
        successfullyChecked += 1

if __name__ == "__main__":
    if args.virustotal and args.abuse:
        print("Not using any APIs, exiting...")
        exit()

    if not args.virustotal:
        virustotalApiKeys = list(set(readFunc("api-keys/vt-apikeys.txt")))

    if not args.abuse:
        abuseApiKeys = list(set(readFunc("api-keys/abuse-apikeys.txt")))
    ...
    print(
        f"Successfully checked on {successfullyChecked} values out of {totalCounter}\n"
    )
    ...
    if args.report:
        print("\nOutput is also created as " + reportName + " in the same directory")
